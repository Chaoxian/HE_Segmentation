...
def highest_mag(slide):
    ...
    """
    return  40 if int(np.floor(float(slide.properties['openslide.mpp-x']) * 10)) == 2 else 20
...
def get_amp_scale(slide, mag):
    largest = highest_mag(slide)
    best_mag = get_level_mag(slide,get_level_for_mag(slide,mag))
    return largest/float(best_mag)
