...
class CustomUNet(nn.Module):
    ...
    def forward(self, x):
        print("1",x.shape)
        encoder_out1 = self.encoder(x)
        print("2",encoder_out1.shape)
        encoder_out2 = self.middle(encoder_out1)
        print("3",encoder_out2.shape)
        decoder_out = self.decoder(encoder_out2)
        print("4",decoder_out.shape)
        x = self.upconv1(decoder_out)
        print("5",x.shape)       
        x = torch.cat([x, encoder_out1], dim=1)
        print("6",x.shape)
        x = self.upconv2(x)
        print("7",x.shape)
        x = self.final_conv(x)
