...
# image_path="/dssg/home/acct-zhaochaoxian/zhaochaoxian-user1/data/dataset/val/F21-5777F_47616_44288_47872_44544.png"
    ...
    # cv2.resize(segmented_image,(int(256*1.5),int(256*1.5)))
    img_name=os.path.split(image_path)[1]
    cv2.imwrite(os.path.join("/dssg/home/acct-zhaochaoxian/zhaochaoxian-user1/InstanceSegment/pathology_maskrcnn/result",name,f"seg_{name}.png"),segmented_image)
    # cv2.imwrite(os.path.join(view_result_path,f"out_{img_name[:-4]}.png"),image_outline)
    # fig, axes = plt.subplots(1, 3, figsize=(10, 5))  # 1 row, 2 columns

    # axes[0].imshow(original_image, interpolation='bicubic')
    # axes[0].set_title('Ori')
    # axes[0].axis('off') 

    # axes[1].imshow(segmented_image, interpolation='bicubic')
    # axes[1].set_title('Pred')
    # axes[1].axis('off') 

    # axes[2].imshow(image_outline, interpolation='bicubic')
